select
        (select b2.real_name from xxx.TAU_USER_INFO b2 where a.RECOMMENDER_MSG = b2.RECOMMEND_NO ) 推荐人,
	...
	(select h.location from YS_PHONE_LOCATION h where h.phonenum=b.phone) 地区,
        ...
        '"'||(select e.CARD_ID from xxx.TAU_USER_DETAILS e where e.user_id=b.user_id )||'"' 身份证,
        a.REGISTRY_TIME 注册时间
        from 
        xxx.TAU_USER_REGISTRY a,
        xxx.TAU_USER_INFO b
